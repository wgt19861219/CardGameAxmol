...
local function getMainButtonTouchConfig(self, key)
	...
	local function clickHandler()
		lsr:report("clickMainButton", {key = key})
			print("[MAIN-BTN] clicked: " .. tostring(key))
	...
	local function handler(event, x, y)
		...
		elseif event == "moved" then
			...
			if dragMode then
				...
				self:refreshMapPos()
				local s = (dt > 0) and (mdx / dt) or 0
				if s ~= 0 and speed / s <= 0 then
					speed = s
				else
					speed = (s + speed) / 2
				end
	...
	local function handler(event)
		xpcall(function()
			if event == "enter" then
				...
				self:refreshButtons()
				self:refreshMailbox()
				LegendLog("[main|onEnter] after_refreshMailbox")
				...
				LegendLog("[main|onEnter] before_refreshTavernTag")
				self:refreshTavernTag()
				LegendLog("[main|onEnter] after_refreshTavernTag")
				...
				LegendLog("[main|onEnter] before_doTeachTavernEnch")
				...
				LegendLog("[main|onEnter] ENTER_DONE")
			end
			if event == "exit" then
...
local function createMainFca(self)
	...
	for k, v in pairs(res_pos) do
		if v.res then
			...
			if node then
				if v.default_action and node.setAction then
					node:setAction(v.default_action)
				end
				if node.setScale then node:setScale(v.scale or 1) end
			self:addFca(node)
			if node.setPosition then node:setPosition(v.pos or ccp(0, 0)) end
			local parent
			...
			if isGap or isLoop then
				local function handler(dt)
					...
					if self.fcaGaps[k] < 0 then
						if node.setAction then node:setAction("Start") end
						resetGap()
					end
...
local function createBottomMap(self)
	...
	--]]
	--����spine����
	-- Try Spine animation; fall back to static background if unavailable
	local spineOk = false
	local spineContainer4 = SpineContainer:create('spine/main_bg_mountain', 'main_bg_mountain')
	if spineContainer4 then
		local spineNode4 = tolua.cast(spineContainer4, 'CCNode')
		if spineNode4 then
			local contentSize = spineNode4:getContentSize()
			if contentSize and contentSize.width > 1 and contentSize.height > 1 then
				spineOk = true
	...
	-- Spine unavailable: use static background + solid blue fill
	if not spineOk then
		print('[MAIN-BG] Spine unavailable, using static background')
		-- 底层蓝色填充，足够宽以覆盖视差偏移
		local bgLayer = CCLayerColor:create(ccc4(40, 100, 180, 255))
		if bgLayer then
			bgLayer:setContentSize(CCSizeMake(2400, 480))
			bgLayer:setAnchorPoint(ccp(0, 0))
			bgLayer:setPosition(ccp(-400, 0))
			container:addChild(bgLayer)
		end
		-- 主背景图：保持原始宽度 2378px，仅缩放高度到 480
		local bgSprite = CCSprite:create('UI/alpha/HVGA/main_bg_mountain.jpg')
		if bgSprite then
			bgSprite:setAnchorPoint(ccp(0, 0))
			local sz = bgSprite:getContentSize()
			if sz.width > 0 and sz.height > 0 then
				bgSprite:setScaleY(480 / sz.height)
				bgSprite:setPosition(ccp(-200, 0))
			end
...
local function createTopMap(self)
	...
	if liziLayer and liziLayer.setPosition then
	...
	if spineContainer4 then
		...
		if spineNode4 then
			...
			container:addChild(spineNode4, 2, 0)
		end
	end
...
local function playBlackCloudAnim(self)
	for _, key in ipairs({"cloud4", "cloud5", "cloud6"}) do
		local cloud = self.ui[key]
		if cloud and not tolua.isnull(cloud) then
			local cdata = res.cloud_res[key]
			if cdata then
				local m1 = CCMoveBy:create(cdata.move_duration, cdata.move_distance)
				local m2 = m1:reverse()
...
local function create(addition)
	...
	LegendLog("[main.create] 1 START")
	SeverConsts:getInstance():setIsInLoading(false);
	--by chenpanhua
	LegendLog("[main.create] 2 before LegendUpdateSvr")
	...
	LegendLog("[main.create] 3 before base.create")
	...
	local ui = self.ui
	LegendLog("[main.create] 4 before createBottomMap")
	self:createBottomMap()
	LegendLog("[main.create] 5 before createMiddleMap")
	self:createMiddleMap()
	LegendLog("[main.create] 6 before createTopMap")
	self:createTopMap()
	LegendLog("[main.create] 7 before createVeryTopMap")
	self:createVeryTopMap()
	LegendLog("[main.create] 8 before createMainButton")
	self:createMainButton()
	LegendLog("[main.create] 9 before createMainFca")
	self:createMainFca()
	LegendLog("[main.create] 10 before createRedLight")
	createRedLight(self)
	LegendLog("[main.create] 11 before mapWidth")
