...
if ed.config.localMode then
  local_server = require("local_server")
  local_server.init()
end
require("mycrypto")
...
local function send(obj, ttype, block)
	...
	if ed.config.localMode then
		...
		return local_server.handle(ttype, obj)
	end
...
local function recv_remain(proto_head_str)
	...
	if allRemainlen > 0 then
		local buffer, err = connection:receive(allRemainlen)
		if buffer then
			LegendLog("    recv_remain " .. allRemainlen .. " bytes")
		end
	end
end
...
local function dispatch(msg)
	...
	if msg._login_reply then
		...
		if user._praise ~= "" then -- url ��ַ
			apprIsVisiable = true
			praise_url = user._praise
		else
			apprIsVisiable = false
		end
		if user._facebook_follow == 0 then --0��δ��ע 1 �ǹ�ע
			fb_flag = true
		else
			fb_flag=false
		end
...
local function OnSendMsgFail()
	...
	elseif reConnectTimes < 8 then--add by xinghui:��ʾͣ������
		serverInUpdate()
	else
		reConnectTimes=0
		--ȥ��¼ҳ��
