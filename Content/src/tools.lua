...
function setimplements(class, ...)
  ...
  for i = 1, amount do
    local tar = select(i, ...)
    if not tar then goto continue_setimplements end
...
local function containsPoint(element, ex, ey)
  ...
  local rx = rect.origin and rect.origin.x or rect.x or 0
  local ry = rect.origin and rect.origin.y or rect.y or 0
  local rw = rect.size and rect.size.width or rect.width or 0
  local rh = rect.size and rect.size.height or rect.height or 0
  return ex >= rx and ex <= rx + rw and ey >= ry and ey <= ry + rh
end
...
local isPointInRect = function(rect, x, y, element)
  ...
  local rx = pos and pos.x or rect.x or 0
  local ry = pos and pos.y or rect.y or 0
  local rw = size and size.width or 0
  local rh = size and size.height or 0
