...
local function createLoading(self)
  LegendLog("[createLoading] START")
  self:refreshExplanation(T(LSTR("LOGO.LOADING_RESOURCES_PLEASE_WAIT……")))
  LegendLog("[createLoading] refreshExplanation done")
  local delay = CCDelayTime:create(0)
  LegendLog("[createLoading] CCDelayTime:create(0) done, delay=" .. tostring(delay))
  local func = CCCallFunc:create(function()
    xpcall(function()
      ...
      if not ed.tutorial.checkDone("_5v5Anim") then
        ...
        LegendLog("[createLoading] about to call ed.ui.main.create()")
        local mainScene = ed.ui.main.create()
        LegendLog("[createLoading] main.create returned, about to replaceScene")
        ed.replaceScene(mainScene)
...
local function createBar(self)
  ...
  local lightLayer = ax.ClippingNode:create()
  lightLayer:setAlphaThreshold(1.0)
  local stencil = CCLayerColor:create(ccc4(255, 255, 255, 255))
  stencil:setContentSize(CCSizeMake(400, 480))
  stencil:setPosition(ccp(15, -240))
  lightLayer:setStencil(stencil)
